...
str(dt)
setkey(dt, "Package", "LibPath", "Version")
key(dt) # [1] "Package"
...
```{r}
uniqueN(dt$Priority)
#dt[,.(Package, LibPath, Version),  by=Priority]
dt[,.N,  by=Priority]
dt[!is.na(Priority),.N,  by=Priority]
...
### List packages in both directories, 9
```{r}
# use inner_join
dt_user[dt_sys, on=.(Package == Package), nomatch=NULL][, c(1,3)] # 9
dt_sys[dt_user, on=.(Package == Package), nomatch=NULL][, c(1,3)] # 9

# !=  is NOT allowed
if false dt1[dt2, on=.(Package != Package)][, 1]

# anti-join, list all packages in dt_user, not in dt_sys
dt_user[!dt_sys, on = .(Package)][, c(1:3)] # 228

# anti , packages in dt_sys, not in dt_user
dt_sys[!dt_user, on = .(Package)][, 1:3] # 20
