...
### candidates
```{r}
cand = as.data.table(old.packages())

```
### update immediately
```{r}
cand[LibPath == .libPaths()[[1]],.(Package, LibPath)]
```

### .libPaths()[[2]]
```{r}
# of all candidates in 2, which have NO installed in 1?
cand[dt, on=.(

#   which already ahve in 1?( inner join)
cand[LibPath == .libPaths()[[2]], .(Package, LibPath)]

# any of candidates have NO match in 1?
