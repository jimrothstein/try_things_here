...
When I update my R packages, I find I have a little twist. I have two libraries;  one for   *standard packages* and one for *user packages* that I installed.  To my suprise,  a few *standard packages* are in both !

(In my case, the directory for *stdlib* is read-only.   When I upgrade, R asks if I want to use user direcotry.   There are many workarounds for this.  RStudio seems to handle the updates fine.  I could change the permissions on the *stdlib* directory, but I do not want to.)

The work-around here is exercise in  comparing two sets of packages before upgrading.
...
### Important to note these are matrix objects and not data.frame
```{r}
is.matrix(I1)
is.matrix(I2)

# expect no package overlap, but ...
P2 = I2[,1:2] ; packages installed sys

I1[I1[, "Package"] %in% P2[,1],1:2, drop=F]

# check
"class" %in% I1[, 1] && "class" %in% I2[, 1]
```
...
## all U1,2 elements of I1,2? (or NULL)
U1[, 1] %in% I1[, 1]
U2[, 1] %in% I2[, 1]
```
...
### Candidates U2 in sys and need to be more careful.
# --------------------------
# Case I 
