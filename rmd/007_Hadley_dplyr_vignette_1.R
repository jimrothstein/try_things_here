...
delay %>% ggplot(aes(dist,delay)) + geom_point()
...
flights %>% distinct(dest) %>% arrange(dest) # alpha order

# next, returns columns 8, 1, 3 control by select; 
#Note: .keep_all
flights %>% distinct(dest, .keep_all = TRUE) %>% 
        select(c(8,1, 3)) %>% head()

# .keep_all default=FAlse
flights %>% distinct(dest, .keep_all = TRUE) %>% head() # keeps all fields
flights %>% distinct(dest ) %>% head()   # returns only 1 field
