...
quote can be:
- convert to list:  use as.list()
- returns typeof:   symbol, lanuage,   
- returns mode(): name, call, 
- does not return R object `expression`
- no evaluation
- no substitution
- print() will not force eval
- not a char[], not atomic  (unless arg is string, in quotes)
...
e |> typeof()   # language
e |> mode()  # call
as.list(e)    # list [[1]] `+` [[2]] x [[3]][1] 2
lobstr::ast(e)

# quote returns list-like, but not a list                                        #
is.list(e)   #F
e[[1]]   # `+`  ie 1st elment



e <- quote(x + y + 2*z)  # typeof language, mode: call
e <- quote("x + y + 2*z")
e |> typeof()   # character surprise?!   

x = 6
quote(x)   # x
identical(quote(x), y <- quote(x)) # [1] TRUE   # no substitution
...
# environment does change eval
z = letters[1:5]
e = quote(z)
eval(e)   # a, b, c ...
identical(eval(e), eval(quote(letters[1:5])))   # TRUE


# but context does change eval !
eval(e, envir=list(z = letters[11:15])) # k, l, m ...
```

# Does quote(...) ever return typeof expression? 
...
more complicated, look at components

returns language (typeof) and call (mode)
```r
e = quote(plot(x=age, y= weight))
e   # prints plot(x=age, y = weight)
# list-like
e$x
e$y
e$x |> typeof()   # symbol
