...
collapse=TRUE) 
```
# works by defusing expression and evaluating in env (from mtcars)
```{r}
base::with(mtcars, mean(cyl))                 # 6.19
base::with(list(cyl=c(10, 20)), mean(cyl))    #15
```
