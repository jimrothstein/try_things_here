...
l = c("typeof", "is.expression", "is.call", "is.symbol")
sapply(l, do.call, list(res))   # expression,  T, F,  F
is.character(res)   # trivial, but take note!


```
## instead of parse, use quote()
```{r}
string <- "2^2"
typeof(string) # [1] "character"

is.call(quote(2*3))
is.call(quote(parse(2*3)))
res2 <- quote(string)
res3 <- quote(parse(text = string))
sapply(l, do.call, list(res2))   # not expression, it is char
sapply(l, do.call, list(res3))   # expression
res3 |> is.call()

is.call(res2)   # F
is.call(res3)   # T

l2 = list("typeof", "is.call")
sapply(l2, do.call, list(res3)) 
```
