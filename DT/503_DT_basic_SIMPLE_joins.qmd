...
## Of the 4 with y=2, 2 are ALSO in 1
## add 3 rows, XYZ that are in both  

    dt = rbind(dt, data.table(x = LETTERS[9:10],y= c(1,1)))
    setkey(dt, x,y)
    dt
...
# pkg in 1, with entry in 2 
    dt2[dt1, on=.(x=x),  nomatch=NULL]

# pkg in 2, with entry in 1
    dt1[dt2, on=.(x=x), nomatch=NULL] 
    dt1[dt2, on=.(x=x), nomatch=0] # same as NULL

# pkg in 2, with NO entry in 1
    dt1[dt2, on=.(x=x)] [is.na(y)]
