...
g2 <- g1 +
  geom_text(
    data = outlier, aes(
      label = paste0(model, class), vjust = 1,
      hjust = 1,
      fontface = "bold"
    ),
    color = "red", size = 3
...
# see legend
g <- ggplot(data = class, mapping = aes(
  x = class, y = hwy,
...
ggplot(diamonds, aes(carat, price)) +
  geom_point()
ggplot(aes(log(brainwt), log(bodywt)), data = msleep) +
  geom_point()
str(msleep) # brainwt, bodywt both num
...
#   # background points
data <- data.frame(
  x = 1:6, # Create example data frame
  y = c(5, 3, 4, 8, 2, 3)
)
#   base plot
...
data_lines <- data.frame(
  x = c(2, 2, 2), # Create data for multiple segments
  y = c(2, 3, 4),
  xend = c(4, 4, 4),
  yend = c(2, 3, 4),
