...
2-examine "mpg"
```{r}
mtcars %>% arrange(desc(mpg)) %>% head(12)

# gives same
mtcars %>% group_by(cyl) %>% arrange(desc(mpg)) %>% head(12)
mtcars %>% group_vars() # character(0)

# count for each group
mtcars %>% group_by(cyl) %>% dplyr::summarize(count = n())
...
# TODO	;error	
mtcars %>% group_by(cyl,vs) %>% summarize(distinct = distinct( ))
...
# add_tally, mutates
mtcars %>% group_by(cyl) %>% add_tally()   # 32 x 12
...
x<-2
base::switch(x, "first", "second")

y <- "10,000 .. 20,000"	
y <- "0 .. 10,000"	
switch(y,"0 .. 10,000" = 3, "10,000 .. 20,000" = 4)
