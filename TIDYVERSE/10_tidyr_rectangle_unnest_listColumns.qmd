...
### PURPOSE:  jennybc examples of tidyr
...
# --------------- get structure  TODO:  ---------------------------------------------
is.list(gh_users) # [1] TRUE
g(gh_users) # list of 6
g(gh_users[[1]]) # list of 30

gh_users[[1]][1:3] # 1st 3
...
## first element of list is first user
one <- gh_users[[1]]
length(one) # 30
#
# ---------------  create tibble  ---------------------------------------------
...
##  only 1 column, but want the first row
glimpse(users$user[1])


# OR,  trial & error, but tighter control
str(users$user, max.level = 1, list.len = 3) # not deep enough
str(users$user, max.level = 2, list.len = 3)
str(users$user[[1]], max.level = 1, list.len = 50) # just first user
# ------------------------------------------------------------
#
#
users
...
names(users[1])
##  hoist:  select only specific cols to break out,  remainer put in last col
(x <- users %>% hoist(user,
...
repos <- repos %>% unnest_longer(repo) # 176  x 1
dim(repos)


##  some probing
repos
str(repos$repo[[1]], list.len = 68, max.level = 1)
