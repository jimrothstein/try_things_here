...
#### Example:  Given a `name` (or `symbol`) get value of object, get()
```r
# mtcars returns its value
mtcars

# but `mtcars` is not a name
is.name(mtcars)  # FALSE
is.name("mtcars") # FALSE

# Lets set or assign Y as the name
Y = as.name("mtcars")
is.symbol(Y)  # T

str(Y)   # symbol mtcars

# so Y is symbol, its value retrieve by get()
get(Y)


# substitute returns symbol, deparse gives it as character
f = function(z) deparse(substitute(z))
X = mtcars
# get accpts character and returns object
get(f(X))  # returns value

identical(get(Y), get(f(X)))   # TRUE
```
...
## Given a tibble, change value in 1 column (by program)
```r
# :=  means evaluate and then set equal
# Goal: change value of arbitrary column
df = tribble(
    ~year,   ~"first_col", ~"second_col", ~"third_col",
    2019,     20,   30, 40)

df
# here:   only first columnn
key = "first"

df |>
    mutate(!! paste0(key, "_col") := 40)
```


## Break it down
# Assemble a symbol (or name) from a string
```{r}
var <- "foo"
sym(var)   # foo
...
## for admiral, works by creating call 1st, then eval
```r
# works (do not wrap :: in ` `)
z=call("::", "pharmaverseadam", "adae")
is.call(z) # T
eval(z)  # prints database
...
f = function(adam) {call("::", "pharmaverseadam", adam)}
f(adam)
eval(f(adam))
...
ds = c("adae", "adsl")
call("::", "pharmaversadam", sym(ds))
eval(call("::", "pharmaversesdtm", sym(ds)))

adam = "adae"
f = function(ds) { call("::", "pharmaverseadam", ds)}
g = function(ds) { call("::", "pharmaverseadam", sym(ds))}
eval(f(adam))
eval(g(adam))

z=eval(lapply(c("adae", "advs"), f))
w=eval(lapply(c("adae", "advs"), g))
...
# ds singular
f = function(ds) {
    z = call("::", "pharmaverseadam", sym(ds))
    }
q = f("adae")
save(q, file="junk.rda")
dir()

eval(get(load(file="junk.rda")))

?save
#    save(eval(z), file = "junk.rda") 
## this will fail
k`::`(pharmaversesdtm, ae, ts)
