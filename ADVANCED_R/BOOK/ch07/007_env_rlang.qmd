...
Nov 23,2024
- Goal:  (1) Annotate, not repeat,  book  (2) collect useful rlang:: functions related to env()
...
rlang::env_print()   # more details
rlang::env_names(rlang::current_env())  # names
```


## 7.2.2 important env
```{r}
rlang::global_env()   # return global
rlang::current_env()   # return current env
identical(global_env(), current_env())


## 7.2.3 parents
rlang::env(a=1)                        # create
rlang::env_parent(rlang::env(a=1))     # find parent 

# create env a whose parent is emptenv()

a=rlang::env(emptyenv(), x=2)          # a is child of TOP parent emptyenv()
env_parent(a) # <environment: R_EmptyEnv>
a$x

## create two env, b has parent a 
a <- env(d = 4, e = 5)      # in global
b <- env(a, a = 1, b = 2, c = 3)   # e2b is child


# check
env_parent(b)   #<environment: 0x55f51af331c8> 
env_parent(a)   # global


env_parents(b)   # <environment: 0x55f51af331c8>), then global
env_parents(a)   # gloval


# FULL envir stack:
E = env_parents(b, last=empty_env())
P = search()

# environment + attr name, path
E[[4 ]]
P[[4]]
...
```{r}
f = function(env=NULL)
  env_parents(env , last=empty_env())
f(current_env())   # omits global (current_env)
```

## 7.3

### call_env() returns an env
```{r}
a = 10
...
### where:  recurse oveer environments
```{r}
...
# env where "lm" found
env_print(where(name = "lm"))  # use string
```
...
### Compare environment names vs environment themself
```{r}
search() # returns name of loaded packages environments, character vector
rlang::search_envs()  # returns the env themselves list

# example returns 3 env in search plath (here stats)
e  <- rlang::search_envs()[[3]]
e   # env stats, attr 
```
...
lobstr::ast(expr(function(x) x + y))
lobstr::ast(function(x) x + y)


# new env; new function g
e  <- env()
e$g  <- function() 1
