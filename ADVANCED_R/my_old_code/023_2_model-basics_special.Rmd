...
For each model (intercept, slope), return tibble.
Many models?  call function again.

```{r}
calc_model <- function(intercept, slope, data) {
  x <- data$x
  y_hat <- intercept + slope * data$y
  residue <- y_hat - y
  mse <- calc_mse(residue)
  ## t <- tibble  (will have list columns)
}
```

```{r}
calc_mse <- function(residuals) {
...
ggplot(models, aes(a1, a2)) +
  geom_point(
    data =
      filter(models, rank(dist) <= 10), size = 4, colour = "red"
  ) +
  geom_point(aes(colour = -dist)) # cute, adds layer, ON top of red dots
```
